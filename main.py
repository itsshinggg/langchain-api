...
def gpt(user_prompt:Prompt):
    """
    GPT endpoint that generates responses using a GPT-based model based on user prompts.
    
    Arguments:
    - user_prompt (str): User-provided prompt message.
    
    Returns:
    - dict: A dictionary containing the response generated by the model {"response": bot answer}.
    """
...
def rag(user_prompt:Prompt):
    """ 
    Endpoint for providing internship-related information based on user queries. Loads internship documents, processes them into embeddings for semantic understanding, and uses a predefined template to generate concise responses to user prompts regarding internship opportunities at City University of Seattle. 
    
    Arguments: 
    - user_prompt (Prompt): User-provided prompt for internship-related queries. 
    
    Returns:
    - dict: Response dictionary containing the generated answer based on the user prompt. 
    """
...
def ragas():
    """
    Endpoint for evaluating internship-related questions against ground truths.

    Returns:
    - None: Generates a CSV output with evaluation metrics.
    """
    loader = TextLoader("./intern.txt")
    docs = loader.load()
